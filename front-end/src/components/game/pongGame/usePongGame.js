...
export default function usePongGame(pongCanvas) {
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'ArrowLeft') keysPressed.value.left = true;
    if (event.key === 'ArrowRight') keysPressed.value.right = true;
  };

  const handleKeyUp = (event) => {
    if (event.key === 'ArrowLeft') keysPressed.value.left = false;
    if (event.key === 'ArrowRight') keysPressed.value.right = false;
  };
  ...
  const handleTouchStart = (event) => {
    ...
    if (x < gameState.value.player1_x) {
  ...
  const updateGame = (deltaTime) => {
    const paddleSpeed = 0.01;
    if (keysPressed.value.left) gameState.value.player1_x = Math.max(gameState.value.player1_x - paddleSpeed, 0);
    if (keysPressed.value.right) gameState.value.player1_x = Math.min(gameState.value.player1_x + paddleSpeed, 1);
  ...
  const checkPaddleCollision = () => {
    if (gameState.value.ball_y >= 0.95 &&
        gameState.value.player1_x - 0.1 <= gameState.value.ball_x && gameState.value.ball_x <= gameState.value.player1_x + 0.1) {
      gameState.value.ball_velocity_y *= -1;
      gameState.value.ball_velocity_x = (gameState.value.ball_x - gameState.value.player1_x) * 0.1;
      ballSpeedFactor.value = Math.min(maxBallSpeed, ballSpeedFactor.value + 5);
    }
  };

  const checkScore = () => {
    if (gameState.value.ball_y >= 1) {
      gameState.value.score1++;
      resetBall();
    }
  };
  ...
  const resetBall = () => {
    gameState.value.ball_x = 0.5;
    gameState.value.ball_y = 0.5;
    gameState.value.player1_x = 0.5;
    gameState.value.ball_velocity_x = 0.01;
    gameState.value.ball_velocity_y = 0;
    ballSpeedFactor.value = initialBallSpeed;
  };
  ...
  const updateCanvas = () => {
    ...
    ctx.fillRect(gameState.value.player1_x * canvas.width - 30, canvas.height - 20, 60, 10);

    ctx.font = '30px Arial';
    ctx.fillText(gameState.value.score1, canvas.width / 2, 30);
  };
