...
export default {
  ...
  methods: {
    ...
    connectToGame(gameMode) {
      ...
      const roomName = gameMode === "join_private" ? this.joinCode : "default_room";
      console.log(`Connecting to WebSocket with room name: ${roomName}`);
      this.gameSocket = new WebSocket(`wss://${window.location.host}/ws/friendPong/${roomName}/`);

      this.gameSocket.onopen = () => {
        console.log("WebSocket connected with mode:", gameMode);
        ...
        console.log("Sending init message:", initMessagePayload);
      ...
      this.gameSocket.onmessage = (event) => {
        ...
        if (messageType === "players_ready") {
          ...
          console.log("Both players connected. Emitting match-ready event.");
          this.$emit("match-ready", { code: this.privateRoomCode, mode: this.currentGameMode, opponent: data.opponent });
        } else if (messageType === "private_room_code") {
          // Update the room code once generated by the backend
          console.log("Private room code received:", data.code);
          console.log("oppoent", data.opponent);
